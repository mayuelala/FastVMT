...
def precompute_freqs_cis_3d(dim: int, end: int = 1024, theta: float = 10000.0):
    # 3D RoPE (Rotary Position Embedding) precomputation
...
def precompute_freqs_cis(dim: int, end: int = 1024, theta: float = 10000.0):
    # 1D RoPE (Rotary Position Embedding) precomputation
...
class SelfAttention(nn.Module):
    ...
    def forward(self, x, freqs, size_info):
        # Create a custom forward function containing all operations
        def custom_forward(x, freqs, size_info):
            ...
            #q_reshaped, k_reshaped = reshape_qk(q, k, size_info)
...
class DiTBlock(nn.Module):
    ...
    def forward(self, x, context, t_mod, freqs, size_info):
        ...
        x = self.gate(x, gate_msa, self.self_attn(input_x, freqs, size_info))
            ...
            if self.training and use_gradient_checkpointing:
                if use_gradient_checkpointing_offload:
                        x = torch.utils.checkpoint.checkpoint(
                            ...
                            x, context, t_mod, freqs, size_info,
                            use_reentrant=False,
                        )
                else:
                    x = torch.utils.checkpoint.checkpoint(
